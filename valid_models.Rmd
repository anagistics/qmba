...
categ_freq <- raw %>% dplyr::select(all_of(categorials)) %>% dplyr::select(-PID) %>% 
    ...
    pivot_longer(cols = everything(), names_to = "Variable", values_to = "Werte")
...
(nvar_var <- raw %>% select_at(all_of(numeric_cols)) %>% dplyr::select(-Order) %>% 
    ...
    pivot_longer(cols = everything(), names_to = "Variable", values_to = "Coeff") %>% 
        arrange(Coeff))
```

Anstelle der Tabelle ist vielleicht auch eine graphische Darstellung nützlich. 
