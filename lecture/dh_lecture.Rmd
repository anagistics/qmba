...
```{r qregr, echo=FALSE}
question("Denken Sie an die Ihnen bekannten Typen von Problemstellungen des maschinellen Lernens. Bei der Problemstellung der Immobilienpreisvorhersage handelt es sich um",
...
```{r qclass, echo=FALSE}
question("Angenommen, das Ergebnis der Software wäre eine Empfehlung zur Annahme oder Ablehnung des Maklerauftrags. Um was für eine Art von Problemstellung würde es dann gehen?",
...
* eine Vorschau auf die Merksmalsausprägungen.
    ...
    **Aufgabe**
    Was bedeutet die Typkennzeichnung `fct` in der Ausgabe von `glimpse`? Was unterscheidet eine *kategoriale* Variable von numerischen? Was wäre der Unterschied, wenn die Variable *Heating* als Zeichenkette (`char` bzw. `character`) abgebildet würde?
    
### Erstes Verständnis der Daten

Mit `glimpse` konnten wir einen ersten Eindruck der Daten gewinnen. Die zahlreichen Einzelwerte bieten aber noch keinen guten Überblick. Dafür ist eine stärker aggregierte Darstellung besser geeignet. Das Paket `dlookr` hilft dabei. Angewandt auf unsere Immobiliendaten erhalten wir:
...
```{r diagnose_missing, exercise = TRUE}
...
```{r missings_nhood, exercise = TRUE}
...
```{r tidyr_mean_imputation, exercise = TRUE}
...
```{r whiteside_remove, exercise = TRUE}
set.seed(123)
...
```{r mice_viz_mean_imputation, exercise = TRUE}
tmp1 <- imp$imp$Gas %>% 
...
```{r mice_lm_imputation, exercise = TRUE}
...
```{r mice_viz_lm_imputation, exercise = TRUE}
tmp1 <- imp2$imp$Gas %>% 
    transmute(i = as.integer(row.names(.)),
              value = `1`)
...
```{r example_rare_values, exercise = TRUE}
...
Aus Histogrammen können wir ableiten, welche Werte je Variable sehr selten auftreten. Gerade bei der Analyse kategorialer Variablen stellt sich oft heraus, dass einige Ausprägungen sehr selten auftreten. Eine statistische Analyse auf Basis dieser Ausprägungen würde statistisch wertlos sein, da die Anzahl der Beobachtungen pro Ausprägung nicht für statistisch valide Aussagen ausreicht. Daher ist es empfehlenswert und üblich, seltene Ausprägungen zu Gruppen zusammenzufassen und eine neue Dummy-Ausprägung, bspw. *sonstige*, für diese einzuführen.
...
```{r count_unique_categories, exercise = TRUE} 
...
```{r find_rare_level_factors, exercise = TRUE}
...
```{r viz_rare_factor_levels, fig.height = 24, exercise = TRUE}
...
```{r define_numeric_columns, exercise = TRUE}
...
```{r viz_numeric_column_densities, fig.height=24, exercise = TRUE}
...
```{r compute_variance, exercise = TRUE}
...
```{r example_correlation, exercise = TRUE}
...
```{r viz_correlations, fig.width = 10, exercise = TRUE}
...
```{r example_linear_model, exercise = TRUE}
...
```{r lm_for_indep_bedroom, exercise = TRUE}
...
```{r lm_for_indep_livarea, exercise = TRUE}
...
```{r example_simple_linear_model, exercise = TRUE}
...
```{r example_three_var_linear_model, exercise = TRUE}
...
```{r example_four_var_linear_model, exercise = TRUE}
...
if (write_mode) {
  prepared_recipe <- housing_rec %>% prep(retain = TRUE)
  saveRDS(prepared_recipe, file = "lecture/www/prepared_recipe.rds")
} else {
...
if (write_mode) {
  housing_fit <- fit(object = lr_mod, formula = formula(prepared_recipe), data = train_prepped)
  saveRDS(housing_fit, file = "lecture/www/housing_fit.rds")
} else {
...
idx_stdres <- tibble::enframe(std_res, name = "Index", value = "StdRes") %>% 
  ...
  bind_cols(train_prepped %>% dplyr::select(SalePrice))
...
if (write_mode) {
  ...
  saveRDS(test_prepped, file = "lecture/www/test_prepped.rds")
} else {
...
pred_truth_test <- tibble::enframe(housing_pred, name = NULL, value = "predicted") %>% 
  bind_cols(test_data %>% dplyr::select(SalePrice)) %>% 
...
Die Vorhersagegüte auf Testdaten bestimmen wir einmal anhand von Metriken und auch graphisch:
...
```{r density_test_errors_vs_training, eval = FALSE}
...
Statistisch kann jedem Koeffizienten $\beta_i$ auch ein Konfidenzintervall zugeordnet werden. Das Paket `sjPlot` bietet die Funktion `plot_model`, das für alle (oder eine Auswahl) von Variablen die Schätzwerte $\hat{\beta}_i$ und deren Konfidenzintervalle darstellt:

```{r chart_model_imp, eval = FALSE}
...
if (write_mode) {
  ...
  saveRDS(prepared_recipe, file = "lecture/www/prepared_recipe_num.rds")
} else {
...
if (write_mode) {
  ...
  saveRDS(housing_fit_num, file = "lecture/www/housing_fit_num.rds")
} else {
...
if (write_mode) {
  ...
  saveRDS(housing_fit_lasso, file = "lecture/www/housing_fit_lasso.rds")
} else {
...
Das Beispiel mit nur numerischen Variablen zeigt, dass man durchaus Gestaltungsmöglichkeiten hat, dabei aber evtl. eine Veränderung der Modellqualität in Kauf nehmen muss. Grundsätzlich ist es wünschenswert ein einfaches und gutes Modell zu haben. Ein einfaches Modell ist dabei (zumindest im Fall der linearen Regression) eines mit wenigen Variablen. Die *LASSO*-Methode (s. @Tibshirani2013), wobei LASSO für *Least Absolute Shrinkage and Selection Operator* steht, ist eine scheinbar kleine Erweiterung der linearen Regression, mit der die zwei Teilprobleme Variablenselektion und Modelloptimierung simultan gelöst werden können: *Regularisierung*.
...
```{r chart_lasso_rmse_lambda, eval = FALSE}
pred_lasso <- predict.model_fit(housing_fit_lasso, new_data = train_prepped)
...
```{r partial_dependence_plot, eval = FALSE}
partial(the_model, train = train_prepped, pred.var = "Garage.Area", ice = TRUE) %>% plotPartial(center = TRUE)
...
### Erklärbarkeit für andere Modelltypen
@Greenwell2018
