...
Analog verfahren wir mit den Testdaten. Hier müssen aber die im `prep`-Schritt ermittelten Parameter (bspw. den Mittelwert bei der Zentrierung oder die Standardabweichung bei der Skalierung) auf die Testdaten anwenden und nicht neu bestimmen:

```{r prep_testing, exercise = FALSE}
...
housing_pred <- predict(housing_fit, test_prepped)
```
...
the_model %>% tidy() %>% filter(term != "(Intercept)") %>% mutate(sign = as.factor(sign(statistic)), imp = abs(statistic)) %>% 
  top_n(n = 15, wt = imp) %>% 
