...
Es gibt zahlreiche Bücher, webbasierte Tutorials und Videos für den Einstieg in R. Dieses Tutorial solld as nicht leisten. Stattdessen verweise ich hier auf eine Reihe von Videos von *Data Science Dojo*, die man hier finden kann: 

![](https://www.youtube.com/playlist?list=PL8eNk_zTBST8j2BU5HYFQogdCjtrHyQAx)
...
```{r glimpse, exercise = TRUE, exercise.setup = "prep_raw"}
glimpse(raw)
```
...
```{r diagnose_all, exercise = TRUE, exercise.setup = "prep_raw"}
diagnose(raw)
```
...
```{r diagnose_num, exercise = TRUE, exercise.setup = "prep_raw"}
...
```{r diagnose_cat, exercise = TRUE, exercise.setup = "prep_raw"}
...
```{r summary, exercise = TRUE, exercise.setup = "prep_raw"}
summary(raw)
```
...
```{r diagnose_missing, exercise = TRUE, exercise.setup = "prep_raw"}
...
```{r derive_types_from_doc, exercise = TRUE}
...
```{r fix_broken_docs, exercise = TRUE}
...
```{r prep_training, exercise = FALSE}
...
```{r density_test_errors_vs_training, exercise = TRUE}
...
```{r table_var_imp, exercise = FALSE}
...
```{r create_numerical_recipe, excercise = FALSE}
...
```{r train_numerical_recipe, exercise = FALSE}
...
```{r setup_lasso_model, exercise = TRUE}
...
```{r lasso_nonzeros, exercise = TRUE}
...
ggplot() + geom_line(data = pred_lambda_rmse, aes(x = log(lambda), y = MSE), color = "blue") +
...
```{r partial_dependence_plot, exercise = FALSE}
