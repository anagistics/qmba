...
housing_variable_types <- function(doc_url = "http://jse.amstat.org/v19n3/decock/DataDocumentation.txt") {
  if (write_mode) {
    ...
    dtab <- data_type_lines %>% str_match_all("^(.+)( |\\s)\\((.+)\\)(\\s*):") %>% compact %>% 
      map(~ c(VAR = .[,2], TYPE = .[,4])) %>% map_dfr(as.list) %>% 
      mutate(VAR = make.names(VAR))
    saveRDS(dtab, file = paste0(dir_pattern, "housing_var_types.rds"))
  } else {
    dtab <- readRDS(file = paste0(dir_pattern, "housing_var_types.rds"))
  }
  dtab
}
...
craw <- raw %>% mutate_at(vars(all_of(categorials)), as.factor) 
...
categ_freq <- craw %>% select(all_of(categorials)) %>% select(-PID) %>% 
...
categ_levels <- craw %>% select(all_of(rel_categs)) %>% map(~ fct_count(.))
...
```{r define_numeric_columns, eval = FALSE}
numeric_cols <- setdiff(names(craw), categorials)
```
...
raw_rs <- craw %>% select_at(all_of(numeric_cols)) %>% select(-Order) %>% 
...
```{r compute_variance}
(nvar_var <- craw %>% select_at(all_of(numeric_cols)) %>% select(-Order) %>% 
...
corr_mat <- craw %>% select(Gr.Liv.Area, Lot.Area) %>% correlate()
```
...
corr_mat <- craw %>% select_at(all_of(numeric_cols)) %>% 
...
linmod <- lm(SalePrice ~ Gr.Liv.Area + Wood.Deck.SF + Bedroom.AbvGr + 
             ...
             data = craw %>% select_at(all_of(numeric_cols)))
...
Ist die Berechnung des Gebäudealters oben unkritisch? Welche Annahme steckt implizit dahinter?
</div>
...
Rufen Sie bitte die Seite https://stats.idre.ucla.edu/r/library/r-library-contrast-coding-systems-for-categorical-variables/#dummy auf und erklären Sie anhand des Beispiels die Dummy-Codierung. 
</div>
...
Der nächste Schritt entfernt die Variablen, die entweder eine sehr kleine Varianz oder eine sehr stark dominierende Ausprägung haben, d.h. wir wenden die Funktion `step_nzv` sowohl auf numerische als auch auf kategoriale oder nominale Variablen an.  
...
* Ist die Varianz der Residuen konstant?
...
Die Anwendung des Modells auf Testdaten ist im Prinzip gleich der Anwendung des Modells im produktiven Betrieb.
Überlegen Sie, welche Vorverarbeitungsschritte, die wir im *Rezept* abgelegt haben, sich vielleicht als 
kritisch erweisen könnten. Denken Sie daran, dass die Anwendung evtl. deutlich nach dem Training des Modells 
erfolgt.
</div>
...
und dann anwenden:
```{r var_imp_plot, exercise = TRUE}
# Rufen Sie die obige Funktion für den Plot der Variablenwichtigkeit für das Modell auf.

```

```{r var_imp_plot-solution}
...
```{r chart_model_imp, fig.height = 15}
...
```{r q-step_rm, echo = FALSE}
frage("Was macht der erste Schritt im obigen Rezept?",
      answer("Löscht alle kategoriale Variablen", correct = TRUE),
      answer("Setzt das Rezept zurück auf Standardwerte"),
      answer("Reserviert Speicher für alle Variablen"))
```
...
```{r train_numerical_recipe_summary, exercise = TRUE}
# Geben Sie ein `summary` für das soeben berechnete Modell aus

```

```{r train_numerical_recipe_summary-solution}
...
Aus der Graphik oben sehen wir, dass unser Modell mit `r length(non_zero_coeffs) - 1` Variablen einen RMSE von ca. 40.000 verglichen mit ca. 30.500 des linearen Modells auf den Trainingsdaten hat. Dafür verwendete das Modell `r length(which(the_model$coefficients != 0))` Variablen. 


## Schlussbemerkung

Damit endet dieses Tutorial. Wir haben einige wichtige Themen der Modellentwicklung und -evaluierung behandelt. Die meisten dieser Themen sind für andere Methoden in ähnlicher Weise durchzuführen. Erfahrungsgemäß wird die Qualität von Modellen durch eine sorgfältige Vorarbeit und Modellvalidierung sehr viel besser als durch den (alleinigen) Wechsel zu einem (vermeintlich) mächtigeren Modelltyp.
...
Dieses Werk steht unter der [Creative Commons Attribution-ShareAlike 4.0 (CC BY-SA .0)](https://creativecommons.org/licenses/by-sa/4.0/)-Lizenz.
